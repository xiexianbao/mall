...
public class BrandServiceImpl implements BrandService {
    ...
    public R<PageData<BrandPageResVo>> pageList(BrandPageReqVo brandPageReqVo) {
        ...
        Page<CommonBrandEntity> page = new Page<>(brandPageReqVo.getPageNum(), brandPageReqVo.getPageSize());
        Page<CommonBrandEntity> res = commonBrandService.page(page, wrapper);

        List<CommonBrandEntity> records = res.getRecords();
        List<BrandPageResVo> brandPageResVos = new ArrayList<>();
        if (!CollectionUtils.isEmpty(records)) {
            brandPageResVos = records.stream().map(record -> {
                BrandPageResVo brandPageResVo = new BrandPageResVo();
                BeanUtils.copyProperties(record, brandPageResVo);
                return brandPageResVo;
            }).collect(Collectors.toList());
        }
        PageData<BrandPageResVo> pageData = PageData.getPageData(res.getCurrent(), res.getSize(), res.getTotal(), brandPageResVos);
